...
## Update 15 Sept 2026 - T2: Tab Dinamis (fondasi multi-tab, lanjutan T1)
- [PROSES] File baru TabBar.kt: pill tab (label "Tab N") + tombol X (sembunyi kalau cuma 1 tab tersisa) + tombol "+" (sembunyi kalau sudah 4 tab). MainActivity.kt: tab list (mutableStateListOf, mulai 1 tab) + activeTabId, homeViewModel sekarang viewModel(key=activeTabId) - tiap tab otomatis dapat instance HomeViewModel sendiri (semua state per-tab dari T1 ikut terpisah otomatis: kategori, Direktori, currentDir, accordion, mode pilih). selectedTab (Beranda/Terakhir) dipindah dari remember lokal ke HomeViewModel (StateFlow baru) supaya ikut per-tab. Bar tab ditaruh sebaris dengan ikon hamburger (sesuai mockup yhs13).
